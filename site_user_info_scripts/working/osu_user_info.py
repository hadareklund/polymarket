#!/usr/bin/env python3
"""Fetch user profile information from osu! (HTML scraper via OG tags)."""
...
def main() -> int:
    ...
    try:
        url = f"https://osu.ppy.sh/users/{args.username}"
        ...
        if not title or "player info" not in title.lower():
            raise RuntimeError("User not found.")
        display_name = title.replace(" · player info", "").strip()
        result = {
            "site": "osu!",
